/*
 * Regenerated BRSTM bundle with Media Chrome adapter (loop + metadata preload)
 *
 * Custom element:
 *   <brstm-audio slot="media" src="song.brstm"></brstm-audio>
 *
 * The original fixed GUI is not initialized.
 * The BRSTM decoding/streaming/playback engine remains in this bundle.
 */
(function () {
    'use strict';
...
<div class="guistate" data-guistate="ready">
    ...
    async function startPlaying(url) { // Entry point to the
        if (!hasInitialized) { // We haven't probed the browser for its capabilities yet
            ...
            hasInitialized = true;
            // Fixed Revolving Door GUI disabled.
            // <brstm-audio> exposes the player through an HTMLMediaElement-like API
            // so Media Chrome (or another custom UI) can control it.
        } // Now we have!
    ...
    // </media-controller>
    //
    // This bundle's BRSTM engine is a singleton, so only one <brstm-audio>
    ...
    function makeTimeRanges(start, end) {
        ...
        return {
            length: valid ? 1 : 0,
            start(index) {
                if (!valid || index !== 0) throw new DOMException("IndexSizeError");
                return start;
            },
            end(index) {
                if (!valid || index !== 0) throw new DOMException("IndexSizeError");
    ...
    class BrstmAudioElement extends HTMLElement {
        static get observedAttributes() {
            return ["src", "loop", "muted"];
        }
        ...
        constructor() {
            ...
            if (!Number.isFinite(this._volume)) this._volume = 1;
            this._volume = Math.max(0, Math.min(1, this._volume));

            this._muted = this.hasAttribute("muted");

            // null = follow the BRSTM file's embedded loop flag.
            // true/false = explicitly overridden through the element API/attribute.
            this._loopOverride = this.hasAttribute("loop") ? true : null;
            this._settingLoopAttribute = false;

            this._lastPaused = true;
            this._lastTime = -1;
            this._lastDuration = NaN;
            this._lastBufferedEnd = -1;
            this._endedSent = false;
            this._pollTimer = null;

            // Common HTMLMediaElement-ish fields queried by media-controller code.
        ...
        attributeChangedCallback(name, oldValue, newValue) {
            ...
            if (name === "src") {
                ...
                // If the attribute was changed externally, treat it as an
                // explicit override. Internal property writes already set
                // _loopOverride before toggling the attribute.
                if (!this._settingLoopAttribute) {
                    this._loopOverride = newValue !== null;
                }

                if (this._ready) {
                    internalApi.setLoop(this._effectiveLoop());
                }
            } else if (name === "muted") {
                const next = this.hasAttribute("muted");
                if (next !== this._muted) {
                    this._muted = next;
                    this._applyVolume();
        ...
        get currentSrc() {
            return this._loadedSrc || this.src;
        }

        get paused() {
            // Before the BRSTM engine has started, behave like a normal <audio>.
            return !this._ready || paused;
        }
        ...
        get duration() {
            const metadata =
                (brstm && brstm.metadata) ||
                this._metadata;

            if (!metadata) return NaN;

            return metadata.totalSamples / metadata.sampleRate;
        }

        get currentTime() {
            if (!brstm || !brstm.metadata) return 0;
            return playbackCurrentSample / brstm.metadata.sampleRate;
        }

        set currentTime(seconds) {
            if (!brstm || !brstm.metadata) return;

            let next = Number(seconds);
            if (!Number.isFinite(next)) return;

            next = Math.max(0, Math.min(next, this.duration));

            // The streaming player cannot safely seek past decoded samples.
            if (Number.isFinite(samplesReady) &&
                samplesReady !== Number.MAX_SAFE_INTEGER) {
                const maxTime = samplesReady / brstm.metadata.sampleRate;
                next = Math.min(next, maxTime);
            }

            this.dispatchEvent(new Event("seeking"));
            internalApi.seek(next * brstm.metadata.sampleRate);
            this._endedSent = false;
        ...
        set muted(value) {
            ...
            this._muted = next;
            this.toggleAttribute("muted", next);
            this._applyVolume();
            this.dispatchEvent(new Event("volumechange"));
        }
        ...
        get loop() {
            return this._effectiveLoop();
        }

        set loop(value) {
            this._loopOverride = Boolean(value);

            this._settingLoopAttribute = true;
            this.toggleAttribute("loop", this._loopOverride);
            this._settingLoopAttribute = false;

            if (this._ready) {
                internalApi.setLoop(this._loopOverride);
            }
        }
        ...
        _effectiveLoop() {
            ...
            // With no explicit HTML/API override, preserve the behavior of the
            // original BRSTM player and honor the file's loop flag.
            if (brstm && brstm.metadata) {
                return brstm.metadata.loopFlag === 1;
            }
        ...
        get readyState() {
            ...
            if (!this._ready || !brstm) {
                return this._metadata ? 1 : 0;
            }

            if (fullyLoaded) return 4;
            if (samplesReady > playbackCurrentSample) return 3;
            return 2;
        }
        ...
        get networkState() {
            ...
            if (
                ...
                (this._loadingPromise && !this._ready)
            ) {
                return 2;
            }
        ...
        get seekable() {
            ...
            if (!Number.isFinite(d)) return makeTimeRanges(0, 0);

            if (fullyLoaded || samplesReady === Number.MAX_SAFE_INTEGER) {
                return makeTimeRanges(0, d);
            }

            const end = Math.min(d, samplesReady / brstm.metadata.sampleRate);
            return makeTimeRanges(0, end);
        }
        ...
        canPlayType(type) {
            ...
            if (
                ...
                t === "audio/x-brstm" ||
                t === "audio/brstm" ||
                t === "application/x-brstm"
            ) return "probably";
            return "";
        }

        load() {
            // Match <audio> semantics: loading metadata must not start playback.
            this._preloadMetadata();
        }

        async _preloadMetadata() {
            const requestedSrc = this.src;
            if (!requestedSrc) return;

            if (
                this._metadata &&
                this._metadataSrc === requestedSrc
            ) {
            ...
            this._metadataPromise = (async () => {
                try {
                    /*
                     * Parse the BRSTM before WebAudio is initialized.
                     *
                     * This fetch is intentionally separate from startPlaying().
                     * In normal browser operation the second request is served
                     * from HTTP cache, so this gives Media Chrome duration
                     * information before the first play gesture without
                     * changing the original playback/streaming engine.
                     */
                    const response = await fetch(requestedSrc);

                    if (!response.ok) {
                        throw new Error(
                            `Failed to load BRSTM metadata: ${response.status} ${response.statusText}`
                        );
                    }

                    const body = await response.arrayBuffer();

                    // Ignore stale responses if src changed while fetching.
                    if (this.src !== requestedSrc) {
                        return null;
                    }

                    const metadataBrstm = new libbrstm.Brstm(body);

                    this._metadata = metadataBrstm.metadata;
                    this._metadataSrc = requestedSrc;
                    ...
                    if (this._metadataSrc === requestedSrc) {
                        this._metadataPromise = null;
                    }
                }
            })();
        ...
        async _ensureStarted() {
            if (this._ready && this._loadedSrc === this.src) return;
            if (this._loadingPromise) return this._loadingPromise;
            if (!this.src) throw new DOMException("No BRSTM src specified", "NotSupportedError");
            ...
            this._loadingPromise = (async () => {
                try {
                    await startPlaying(requestedSrc);

                    this._loadedSrc = requestedSrc;
                    this._ready = true;
                    this._starting = false;
                    this._endedSent = false;
        ...
        async play() {
            // If metadata preloading is already in flight, allow it to finish
            // first so Media Chrome has a stable duration before playback begins.
            if (this._metadataPromise) {
                try {
                    await this._metadataPromise;
                } catch (_) {
                    // _ensureStarted() will report the actual playback failure.
                }
            }

            // Important: startPlaying() begins playback itself on first load.
            if (!this._ready || this._loadedSrc !== this.src) {
            ...
            // internalApi.pause() is a toggle in the original player.
            internalApi.pause();

            // Wait for the original audio callback to finish the unpause request.
        ...
        _applyVolume() {
            ...
            internalApi.setVolume(this._muted ? 0 : this._volume);
        }
        ...
        _startPolling() {
            ...
            this._pollTimer = setInterval(() => {
                ...
                const nowDuration = this.duration;

                if (Number.isFinite(nowDuration) && nowDuration !== this._lastDuration) {
                    this._lastDuration = nowDuration;
                    this.dispatchEvent(new Event("durationchange"));
                }
                ...
                if (nowPaused !== this._lastPaused) {
                    const wasPaused = this._lastPaused;
                    this._lastPaused = nowPaused;

                    if (nowPaused) {
                        this.dispatchEvent(new Event("pause"));

                        // The original non-looping EOF path resets the sample
                        // position to 0 and requests a pause.
                        if (
                            wasPaused === false &&
                            !enableLoop &&
                            this._ready &&
                ...
                if (this._ready && brstm && brstm.metadata) {
                    const end = fullyLoaded || samplesReady === Number.MAX_SAFE_INTEGER
                        ? this.duration
                        : Math.min(this.duration, samplesReady / brstm.metadata.sampleRate);

                    if (end !== this._lastBufferedEnd) {
                        this._lastBufferedEnd = end;
                        this.dispatchEvent(new Event("progress"));
                    }

                    if (
                        !fullyLoaded &&
                        samplesReady <= playbackCurrentSample
                    ) {
    ...
    if (!customElements.get("brstm-audio")) {
        customElements.define("brstm-audio", BrstmAudioElement);
    }
